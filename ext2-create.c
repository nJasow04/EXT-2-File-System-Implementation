...
void write_superblock(int fd) {
	...
	superblock.s_r_blocks_count = 0;							// Set to 0 because super user doesn't need to reserve shit
	superblock.s_free_blocks_count = NUM_FREE_BLOCKS;
	superblock.s_free_inodes_count = NUM_FREE_INODES;
	superblock.s_first_data_block = 1; /* First Data Block */			// Superblock is exactly 1 KiB Large, therefore 1
	superblock.s_log_block_size = 0;					/* 1024 */ 	// block size = 1024 << s_log_block_size;
	superblock.s_log_frag_size = 0;						/* 1024 */	
	superblock.s_blocks_per_group = NUM_BLOCKS;				// assuming only 1 block group
	...
	superblock.s_max_mnt_count     = -1; /* Make this unlimited */ // -1 = unlimited
	superblock.s_magic = EXT2_SUPER_MAGIC; /* ext2 Signature */		// 0xEF53
	superblock.s_state             = 1; /* File system is clean */ // 1 = clean
	superblock.s_errors            = 1; /* Ignore the error (continue on) */ // 1 = continue on
	superblock.s_minor_rev_level   = 0; /* Leave this as 0 */
	superblock.s_lastcheck = current_time; /* Last check time */
	superblock.s_checkinterval     = 1; /* Force checks by making them every 1 second */	// Checks every 1 second
...
void write_block_bitmap(int fd)
{
	...
	// First 23 bits are initially used
	int used_blocks = LAST_BLOCK; //INODE_TABLE_BLOCKNO + 16 /* Number of blocks used for inode table */ - 1;
...
void write_inode_bitmap(int fd)
{
	...
	// TODO It's all yours
	u8 map_value[BLOCK_SIZE] = {0};			// Initialize everything to 0

	int used_inodes = LAST_INO; // LOST_AND_FOUND_INO;			// First 11 Inodes are reserved on Start-Up
...
void write_inode_table(int fd) {
	...
	root_directory_inode.i_links_count = 3;			// . .. and LOST+FOUND ..
	root_directory_inode.i_blocks = 2; /* These are oddly 512 blocks */
	...
	const char* link_target = "hello-world";
	hello_inode.i_size = strlen(link_target); // Length of the path string
	memcpy(&(hello_inode.i_block), link_target, hello_inode.i_size); // Store the path directly in i_block
...
void write_root_dir_block(int fd)
{
    ...
    dir_entry_set(current_entry, LOST_AND_FOUND_INO, "lost+found");
    dir_entry_write(current_entry, fd);
    bytes_remaining -= current_entry.rec_len;
	// printf("rec_len lost+found: %d\n", current_entry.rec_len);

    // Entry for 'hello-world'
    dir_entry_set(current_entry, HELLO_WORLD_INO, "hello-world");
    dir_entry_write(current_entry, fd);
    bytes_remaining -= current_entry.rec_len;
	// printf("rec_len hello-world: %d\n", current_entry.rec_len);

	// Entry for 'hello'
	dir_entry_set(current_entry, HELLO_INO, "hello");
	dir_entry_write(current_entry, fd);
	bytes_remaining -= current_entry.rec_len;
	// printf("rec_len hello: %d\n", current_entry.rec_len);
    ...
    // Fill the rest of the block with zeroes if there's any space left
    // if (bytes_remaining > 0) {
    //     char zero_padding[bytes_remaining];
    //     memset(zero_padding, 0, bytes_remaining);
    //     if (write(fd, zero_padding, bytes_remaining) != bytes_remaining) {
    //         errno_exit("write");
    //     }
    // }
...
void write_hello_world_file_block(int fd)
{
	...
	if (write(fd, content, 12) != 12) {
        errno_exit("write");
    }

	// Safety
	// ssize_t remaining = BLOCK_SIZE - 12;
    // if (remaining > 0) {
    //     char zero_padding[remaining];
    //     memset(zero_padding, 0, remaining);
    //     if (write(fd, zero_padding, remaining) != remaining) {
    //         errno_exit("write");
    //     }
    // }
}
